...
struct LaplaceSolver{
    ...
    LaplaceSolver(Grid *g, int num_threads);
    ~LaplaceSolver();
    void initialize();
...
LaplaceSolver :: LaplaceSolver(Grid *grid, int num_threads)
{
    g = grid;
...
Real LaplaceSolver :: timeStep(const Real dt)
{
    ...
    #pragma omp parallel for shared(u) private(i,j,tmp) reduction(+:err) num_threads(this->num_threads)
    for (i=1; i<nx-1; ++i) {
        for (j=1; j<ny-1; ++j) {
            tmp = u[i][j];
...
int main(int argc, char * argv[])
{
    ...
    cout << "num_threads;nx;result;seconds" << endl;
    for(int i = 0; i < 3; i++){
        for(int j = 1; j <= omp_get_max_threads(); j++ ){
            Grid *g = new Grid(nx[i], nx[i]);
            g->setBCFunc(BC);
            LaplaceSolver s = LaplaceSolver(g,j);
            t_start = omp_get_wtime();
            result = s.solve(n_iter, eps);
            t_end = omp_get_wtime();
            std::cout << j << ";" << g->nx << ";"  << result << ";" << t_end - t_start << endl; 
        }
    }
    return 0;
}